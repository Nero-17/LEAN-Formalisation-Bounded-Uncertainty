"""Check the unchanged first Section 4 baseline, rebuild round two, and audit all declarations.
...
logs = project / ".lake/build/section4-round2-verification"
logs.mkdir(parents=True, exist_ok=True)
baseline = json.loads((project / "verification-section4-round1.json").read_text(encoding="utf-8-sig"))
...
freeze = [{"module": name, "sha256": hashes[name],
          ...
          for name in modules]
(project / "verification-section4-round2-source-freeze.json").write_text(
...
def audit_chunk(index):
    ...
    source = project / f"verification-section4-round2-axioms-{index + 1}-input.lean"
    ...
    (project / f"verification-section4-round2-axioms-{index + 1}-output.txt").write_text(result.stdout, encoding="utf-8")
...
header = (f"Section 4 round two incremental verification started {started_at}.\n"
          f"All {len(baseline_modules)} first-batch source hashes match the preserved baseline.\n"
...
(project / "verification-section4-round2.txt").write_text(header + "\n".join(records) +
