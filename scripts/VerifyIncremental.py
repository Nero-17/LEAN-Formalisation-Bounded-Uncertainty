"""Check an unchanged Section 3 baseline, rebuild every added module, and audit all declarations.
...
Section 3 verification files are read as evidence, never rewritten.
"""
...
logs = project / ".lake/build/section4-verification"
logs.mkdir(parents=True, exist_ok=True)
baseline = json.loads((project / "verification-section3.json").read_text(encoding="utf-8-sig"))
...
freeze = [{"module": name, "sha256": hashes[name],
          ...
          for name in modules]
(project / "verification-section4-source-freeze.json").write_text(
...
def audit_chunk(index):
    ...
    source = project / f"verification-section4-axioms-{index + 1}-input.lean"
    ...
    (project / f"verification-section4-axioms-{index + 1}-output.txt").write_text(result.stdout, encoding="utf-8")
...
header = (f"Section 4 incremental verification started {started_at}.\n"
          f"All {len(baseline_modules)} Section 3 source hashes match the preserved baseline.\n"
...
(project / "verification-section4.txt").write_text(header + "\n".join(records) +
