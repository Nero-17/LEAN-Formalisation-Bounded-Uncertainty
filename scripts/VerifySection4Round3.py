"""Check the unchanged second Section 4 baseline, rebuild round three, and audit all declarations.
...
logs = project / ".lake/build/section4-round3-verification"
logs.mkdir(parents=True, exist_ok=True)
baseline = json.loads((project / "verification-section4-round2.json").read_text(encoding="utf-8-sig"))
...
freeze = [{"module": name, "sha256": hashes[name],
          ...
          for name in modules]
(project / "verification-section4-round3-source-freeze.json").write_text(
...
def audit_chunk(index):
    ...
    source = project / f"verification-section4-round3-axioms-{index + 1}-input.lean"
    ...
    (project / f"verification-section4-round3-axioms-{index + 1}-output.txt").write_text(result.stdout, encoding="utf-8")
...
header = (f"Section 4 round three incremental verification started {started_at}.\n"
...
(project / "verification-section4-round3.txt").write_text(header + "\n".join(records) +
