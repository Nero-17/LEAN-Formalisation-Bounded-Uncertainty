"""Check the unchanged fourth Section 4 baseline, rebuild round five, and audit all declarations.
...
logs = project / ".lake/build/section4-round5-verification"
logs.mkdir(parents=True, exist_ok=True)
baseline = json.loads((project / "verification-section4-round4.json").read_text(encoding="utf-8-sig"))
...
freeze = [{"module": name, "sha256": hashes[name],
          ...
          for name in modules]
(project / "verification-section4-round5-source-freeze.json").write_text(
...
from LeanInventory import declarations

source_names = []
declaration_kinds = {}
for name, path in sources.items():
    for declaration, kind in declarations(path.read_text(encoding="utf-8-sig")):
        source_names.append(declaration)
        declaration_kinds[kind] = declaration_kinds.get(kind, 0) + 1
...
inventory_hash = hashlib.sha256((project / "scripts/LeanInventory.py").read_bytes()).hexdigest()
...
def audit_chunk(index):
    ...
    source = project / f"verification-section4-round5-axioms-{index + 1}-input.lean"
    ...
    (project / f"verification-section4-round5-axioms-{index + 1}-output.txt").write_text(result.stdout, encoding="utf-8")
...
assert hashlib.sha256((project / "scripts/LeanInventory.py").read_bytes()).hexdigest() == inventory_hash
header = (f"Section 4 round five incremental verification started {started_at}.\n"
...
(project / "verification-section4-round5.txt").write_text(header + "\n".join(records) +
...
result = {"verifiedAtUtc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
          ...
          "auditedDeclarationCount": len(names), "declarationKinds": declaration_kinds, "addedModules": new_modules,
