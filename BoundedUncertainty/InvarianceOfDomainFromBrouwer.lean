...
Released under Apache 2.0 license; see third-party/KaiLam-Apache-2.0.txt.
Authors: Kai Lam

Adapted for Lean 4.28.0 from the dependency-closed invariance-of-domain
section of mathlib PR 36770, source snapshot InvarianceOfDomain-230d75a.lean.
The incomplete earlier retraction arguments are not included or imported.
The Brouwer premise below is explicit. InvarianceOfDomain.lean discharges
it using the independently verified cubical-Sperner fixed-point foundation.
-/
...
/-- An explicit Brouwer interface, proved in InvarianceOfDomain.lean.
This structure is not registered as an assumed instance. -/
structure BrouwerFixedPoint (E : Type*) [NormedAddCommGroup E] : Prop where
...
variable (brouwer : BrouwerFixedPoint E)
include brouwer
...
lemma stability_of_zero (f : E → E) (hf_cont : ContinuousOn f (closedBall 0 1))
  ...
  obtain ⟨x, hx⟩ := brouwer.brouwer_fixed_point
...
theorem invariance_of_domain_interior (f : E → E)
  ...
  · have himage : f '' closedBall 0 1 = univ := by
      apply eq_univ_of_forall
      intro point
      exact ⟨0, by simp, Subsingleton.elim _ _⟩
    simp only [himage, interior_univ, mem_univ]
  ...
  have hStability_of_zero (Gtilde : E → E) (hGtilde : ContinuousOn Gtilde (f '' closedBall 0 1))
    ...
    stability_of_zero brouwer f hf_cont G' hG'_left_inv Gtilde hGtilde hy
...
theorem invariance_of_domain_open_map (f : E → E) (U : Set E) (hU : IsOpen U)
  ...
  have h_interior : e 0 ∈ interior (e '' closedBall 0 1) :=
    invariance_of_domain_interior brouwer e he_cont he_inj
