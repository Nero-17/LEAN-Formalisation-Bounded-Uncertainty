...
Lean 4 formalisation of Section 3 and selected results of Section 4 of *Dynamics with state-dependent uncertainty: a boundary map approach*. The current project contains 87 mathematical modules and 637 named declarations. Lean 4.28.0 and an exact mathlib revision are pinned below. Section 3 is complete within the stated deductive scope; Section 4 is in progress.
...
The first Section 4 batch adds 18 modules and 127 declarations. All original 69 Section 3 mathematical source files retain their verified SHA-256 hashes. The current Section 4 scope and independent review are recorded in [section4-coverage-review.md](section4-coverage-review.md).
...
| Theorem 4.9, partial | Source-normal continuity derived from pointwise C1 data; noncircular raw Gamma; forward implication; source-visible range equality; unique contributors from Gamma injectivity; actual frontier homeomorphism under Gamma bijectivity | BoundaryNormalContinuity, BoundaryProjectionTopology |
...
**Still open:** the independent secant-to-C1 criterion in Lemma 4.8; the reverse regularity implication and secant estimates of Theorem 4.9; the analytical dual-ball geometry and nearest-point differential; Theorems 4.16–4.17 and Corollary 4.18. The annulus visibility example and numerical trajectories are not certified by this batch. The completed first-step initialization is not a numerical certificate for later plotted shapes. Section 4 is not claimed complete.
...
Section 4 uses an explicitly incremental verification: hash-check all unchanged baseline sources, freshly rebuild every added module in dependency order, compile the updated root, and run every one of the 637 current axiom queries in three disjoint groups. [scripts/VerifyIncremental.py](scripts/VerifyIncremental.py) also checks that the audit names exactly match all handwritten source declarations. The baseline compiled objects are reused; this run is not described as a fresh rebuild of all 87 modules. See [verification-section4.txt](verification-section4.txt) for the new compiler and axiom output and [verification.json](verification.json) for current hashes, counts, and scope. The standard Lake build commands above still build the entire project from source.
